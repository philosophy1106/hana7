...
update Emp set email=concat('email', id, '@gmail.com');
alter table Emp add index idx_Emp_email (email);

explain select * from Emp where email like 'mail%';
...
alter table Emp add index(
        (substring_indeex(email, '@', -1))
);

#연습 문제
-- E 

alter table Emp add column mobile varchar(11);
alter table Emp modify column monbile varchar(11) not null default '' after salary;

select concat('0101234', lpad(id, 4,'0')) from Emp order by id;
...
explain select * from Emp limit 10 offset 100;
...
alter table Notice add constraint fk_Notice_writer_Emp foreign key (writer)
  references Emp(id) on delete set null;
  
show index from Notice;

CREATE FULLTEXT INDEX ft_idx_Notice_title_contents on Notice(title, contents);
...
explain select * from Notice where match(title, contents) against('이순신');
...
select * from Notice where match(title, contents) against('조선* +문신*' IN BOOLEAN MODE);
